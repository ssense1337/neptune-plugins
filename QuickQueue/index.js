var l=t=>t?.nodeType===Node.ELEMENT_NODE;import{actions as c,store as f}from"@neptune";import{ReactiveRoot as v}from"@neptune/components";import{html as m}from"@neptune/voby";var b=t=>{let e=t.attributes,o={};for(let n=0;n<e.length;n++){let s=e[n];o[s.name]=s.value}return o},$=(t,e)=>{let{elements:o}=f.getState().playQueue,n=o.some(r=>r.mediaItemId===e),s=n?"detail-view__trashcan":"player__queue-add",u=n?"Remove from queue":"Add to queue",a=t.querySelector('button[data-test="add-to-playlist-button"]');if(!a)return null;let g=b(a),q=b(a.querySelector("svg"));return m`
        <button
